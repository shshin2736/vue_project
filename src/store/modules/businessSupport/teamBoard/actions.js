...
export default {
  ...
  //* 팀 게시판 리스트 가져오기
  async [actions.FETCH_TEAM_BOARD_LIST]({ commit }, payload) {
    try {
      //* 팀 게시판 리스트 가져오기
      const { data } = await axiosInstance.post(
        'api/support/getTeamBoardList.do',
        payload,
      );
      //* 팀 게시판 리스트 저장하기
      commit(mutations.SET_TEAM_BOARD_LIST, data.data);
    } catch (error) {
      errorEventHandler(error);
    }
  },
  //* 팀 게시판 내용 가져오기
  async [actions.FETCH_TEAM_BOARD_DETAIL]({ commit }, payload) {
    try {
      //* 팀 게시판 내용 가져오기
      const { data } = await axiosInstance.post(
        'api/support/getTeamBoardDetail.do',
        payload,
      );
      //* 팀 게시판 내용 저장하기
      commit(mutations.SET_TEAM_BOARD_DETAIL, data.data);
      return data;
    } catch (error) {
      errorEventHandler(error);
    }
  ...
  async [actions.UPDATE_TEAM_BOARD_DETAIL]({ dispatch }, payload) {
    try {
      ...
      dispatch(actions.FETCH_TEAM_BOARD_LIST, {});
      return data;
    } catch (error) {
      errorEventHandler(error);
    }
  ...
  [actions.INIT_TEAM_BOARD_DETAIL]({ commit }, payload) {
    commit(mutations.SET_TEAM_BOARD_DETAIL, payload);
  },
